...
@section('masjs')
  ...
  <script src="{{ asset('js/sub_group.js') }}"></script>
@endsection

@section('migasdepan')
    <a href="{{ route('items') }}">{{ __('ITEMS') }}</a>
    &nbsp;&nbsp;<i class="icon ion-android-arrow-forward"></i>&nbsp;&nbsp;{{ __('SUBGRUPOS') }}
              ...
              <table id="tbl_read" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($registros as $registro)
                    <tr>
                      <td>
                        <a href="{{ route('sub_grupos.edit', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();">
                              {{ $registro->name }}
                        </a>
                        <form id="frm_registro_{{ $registro->id }}" action="{{ route('sub_grupos.edit', $registro->id) }}" class="d-none">
                            @method('PUT')
                            @csrf
                        </form>
                      </td>
                      <td>{{ $registro->group["name"] }}</td>
                      <td>
                        @if ($trashed == 0)
                        ...
                        <form method="POST" id="frm_eliminar_{{ $registro->id }}" action="{{ route('sub_grupos.destroy', $registro->id) }}" class="d-none">
                            @method('DELETE')
                            @csrf
