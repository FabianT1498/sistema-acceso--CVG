              ...
              <table id="tbl_read" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($registros->get() as $registro)
                    <tr>
                      <td>
                        <input type="checkbox" id="chm_{{ $registro->id }}" class="chkImp" data-reg="{{ $registro->id }}" checked="checked" />
                      </td>
                      <td>
                        <a href="{{ route('entregas.edit', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();">
                              {{ $registro->control_number }}
                        </a>
                        <form id="frm_registro_{{ $registro->id }}" action="{{ route('entregas.edit', $registro->id) }}" class="d-none">
                            @method('PUT')
                            @csrf
                        ...
                        <td><a href="{{ route('entregas.edit', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();">
                              {{ $registro->description }}
                        </a></td>
                        <td>{{ $registro->delivered_date }}</td>
                        <td>{{ $registro->unity }}</td>
                        ...
                        <td>
                          ...
                          <form method="POST" id="frm_eliminar_{{ $registro->id }}" action="{{ route('entregas.destroy', $registro->id) }}" class="d-none">
                              @method('DELETE')
                              @csrf
                              <input type="hidden" name="search" value="{{ $search }}">
                          </form> 
