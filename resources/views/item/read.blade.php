              ...
              <table id="tbl_read" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($registros->get() as $registro)
                    <tr>
                      <td>
                        <a href="{{ route('productos.edit', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();">
                              {{ $registro->description }}
                        </a>
                        <form id="frm_registro_{{ $registro->id }}" action="{{ route('productos.edit', $registro->id) }}" class="d-none">
                            @method('PUT')
                            @csrf
                        </form>
                      </td>
                      <td>{{ $registro->type->sub_group->group->name }}</td>
                      <td>{{ $registro->type->sub_group->name }}</td>
                      <td>{{ $registro->type->name }}</td>
                      <td>{{ $registro->presentation->name }}</td>
                      <td>
                        @if ($trashed == 0)
                        ...
                        <form method="POST" id="frm_eliminar_{{ $registro->id }}" action="{{ route('productos.destroy', $registro->id) }}" class="d-none">
                            @method('DELETE')
                            @csrf
