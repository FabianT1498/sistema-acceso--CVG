...
@section('migasdepan')
    ...
    &nbsp;&nbsp;<i class="icon ion-android-arrow-forward"></i>&nbsp;&nbsp;{{ __('COMPRAS PENDIENTES') }}
              ...
              <table id="tbl_read" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($registros->get() as $registro)
                    <tr>
                      <td>
                        <a href="{{ route('compras-confirmar', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();">
                              {{ $registro->control_number }}
                        </a>
                        <form id="frm_registro_{{ $registro->id }}" action="{{ route('compras-confirmar', $registro->id) }}" class="d-none">
                            @method('PUT')
                            @csrf
