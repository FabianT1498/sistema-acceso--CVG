              ...
              <table id="tbl_read" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($registros->get() as $registro)
                    <tr>
                      <td>
                        <a href="{{ route('usuarios.edit', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();">
                              {{ $registro->firstname }}
                        </a>
                        <form id="frm_registro_{{ $registro->id }}" action="{{ route('usuarios.edit', $registro->id) }}" class="d-none">
                            @method('PUT')
                            @csrf
                        </form>
                      </td>
                      ...
                      <td>
                        @if ($trashed == 0)
                        <a title="Eliminar" href="{{ route('usuarios-destroy', $registro->id) }}" onclick="return confirm('¿Está seguro? Esta acción no se puede deshacer.')" class="delete text-c-red"><small><small class="text-danger"><i class="fa fa-trash fa-2x"></i></small></small></a>
                        @endif
                      </td>
                    </tr>
                  @endforeach
