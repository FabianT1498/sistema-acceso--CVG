              ...
              <table id="tbl_read" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($registros->get() as $registro)
                    <tr>
                      <td>
                        <a href="{{ route('presentaciones.edit', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();">
                              {{ $registro->name }}
                        </a>
                        <form id="frm_registro_{{ $registro->id }}" action="{{ route('presentaciones.edit', $registro->id) }}" class="d-none">
                            @method('PUT')
                            @csrf
                        </form>
                      </td>
                      <td>
                        ...
                        <form method="POST" id="frm_eliminar_{{ $registro->id }}" action="{{ route('presentaciones.destroy', $registro->id) }}" class="d-none">
                            @method('DELETE')
                            @csrf
