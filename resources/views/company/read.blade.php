              ...
              <table id="tbl_read" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($registros->get() as $registro)
                    <tr>
                      <td>
                        <a href="{{ route('empresas.edit', $registro->id) }}"
                          onclick="event.preventDefault();
                          document.getElementById('frm_registro_{{ $registro->id }}').submit();"
                          title="{{ __('Direccion: ') }}{{ $registro->address }}">
                              {{ $registro->name }}
                        </a>
                        <form id="frm_registro_{{ $registro->id }}" action="{{ route('empresas.edit', $registro->id) }}" class="d-none">
                            @method('PUT')
                            @csrf
                      ...
                      <td>
                        ...
                        <form method="POST" id="frm_eliminar_{{ $registro->id }}" action="{{ route('empresas.destroy', $registro->id) }}" class="d-none">
                            @method('DELETE')
                            @csrf
                            <input type="hidden" name="search" value="{{ $search }}">
                        </form>
